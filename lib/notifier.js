...
class Notifier {
  ...
  notify (title, data) {
    ...
    // Close NotificationWindow if renderer window get's closed	  
    if (this.BrowserWindow.getFocusedWindow()) {	  
      this.BrowserWindow.getFocusedWindow().on('close', function ()  {
        if (notificationWindow && !notificationWindow.isDestroyed()) {
          notificationWindow.close();
	}
      });
    }
    // Replace notification with Tag	  
    for(var i = 0; i < this.activeNotifications.length ; i++) {
      var activeNotification = this.activeNotifications[i];
      if (activeNotification.options && activeNotification.options.tag == options.tag && activeNotification.window && !activeNotification.window.isDestroyed()) {
       activeNotification.window.webContents.send('setup', title, options);
       return activeNotification.window;
      }
    }
  ...
  showNext () {
    ...
    notificationWindow.on('buttonmousemove', (event) => {
